...
**规则与订阅**
- 规则集：内置 14 个（广告拦截、国内域名/IP、局域网、Apple、Google、Telegram、GFW 列表等，来自 Loyalsoldier/clash-rules），也可以自己写内联规则
- **规则由服务端中转**：规则集正文由服务端抓取并缓存在本地，生成的配置里 `rule-provider` 指向本站。
  否则客户端得自己去 `raw.githubusercontent.com` 拉——那地址本来就要先有代理才够得着，而规则正是用来决定怎么代理的。
  同时会把 `proxy` 钉成 `DIRECT`：内核拉 provider 也要过一遍自己的规则，那时规则还没加载，请求会全落到 `MATCH` 被丢进还没就绪的代理组
